...
# Demo mode (optional)
DEMO_ENABLED=true
# Demo accounts are auto-deleted after this many hours (default: 48)
DEMO_TTL_HOURS=48

# Maintenance scripts safety (recommended)
# Some maintenance scripts refuse to run in production unless this is set.
ALLOW_PROD_MAINTENANCE=false
...
MEDIA_ALLOWED_HOSTS=cdn.isntgram.ai,localhost:9000,127.0.0.1:9000,picsum.photos
# Note: the public API contract caps uploads at 5MB. This value may further restrict uploads,
# but increasing beyond 5MB requires a code + contract change.
...
# When deploying behind a reverse proxy, set this to true so Auth.js trusts X-Forwarded-Host.
...
NEXT_PUBLIC_MEDIA_HOSTS=localhost:9000,127.0.0.1:9000,cdn.isntgram.ai,picsum.photos
# Max upload size hint for the UI (bytes)
# Note: UI will clamp this to the API contract max (currently 5MB).
...
# These are demo *trigger* credentials; the API creates a fresh demo user each time.
