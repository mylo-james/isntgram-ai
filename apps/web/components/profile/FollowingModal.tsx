...
export default function FollowingModal({
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await apiClient.getFollowing(username, 1, 20);
        setUsers(res.users);
        setPage(1);
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const res = await apiClient.getFollowing(username, nextPage, 20);
  ...
  return (
    <Modal open={isOpen} onClose={onClose} title={`Following (${followingCount})`} contentClassName="max-h-[80vh]">
