...
describe("social comment like BFF with actual guards", () => {
  const origin = "http://127.0.0.1:4320";
  const originalEnv = process.env;
  const csrf = "social-fixture.1788800000000";
  const requestId = "social-boundary-request";
  const params = () => ({ params: Promise.resolve({ postId: " post-1 ", commentId: " comment-1 " }) });
  ...
  const request = (method: string, headers: Record<string, string> = {}) =>
    new Request(origin + "/api/bff/posts/post-1/comments/comment-1/like", {
      method,
  ...
  describe.each([
    ...
    it.each([
      ...
      expect(internalApi[method]).toHaveBeenCalledWith("/api/posts/{postId}/comments/{commentId}/like", {
        params: { path: { postId: "post-1", commentId: "comment-1" } },
