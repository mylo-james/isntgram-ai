...
| Containerization | Docker & Docker Compose   | Application packaging  | Essential for consistent development and production environments, enabling reliable deployments and local development.                                                                                     |
...
| **State Management**        | NextAuth + local state    | N/A     | Identity + UI state                                       | Auth via NextAuth session; UI state is local (avoid Redux/global stores unless truly shared).                             |
...
| **Database**                | PostgreSQL                | 16.x    | Primary data store for all application data.              | A robust relational database ideal for managing the social graph's data. Fits within managed database free tiers.         |
...
| **Containerization**        | Docker                    | ~24.x   | Application packaging and deployment.                     | Ensures consistent environments across development and production, enabling reliable deployments.                         |
| **Container Orchestration** | Docker Compose            | ~2.x    | Multi-container application management.                   | Simplifies local development and production deployment with service orchestration.                                        |
| **Container Registry**      | GitHub Container Registry | N/A     | Container image storage and distribution.                 | Free, secure, and integrated with GitHub for seamless CI/CD workflows.                                                    |
...
| **CI/CD**                   | GitHub Actions            | N/A     | Automates testing and deployment pipelines.               | A free and powerful CI/CD solution that integrates directly with our code repository and container registry.              |
| **Build Optimization**      | Turborepo                 | ~1.x    | Monorepo build system with caching.                       | Optimizes build performance across the monorepo with intelligent caching and parallel execution.                          |
| **Security Scanning**       | Trivy                     | latest  | Container vulnerability scanning.                         | Ensures container images are secure and free from known vulnerabilities.                                                  |
| **Monitoring & Logging**    | Built-in & Self-hosted    | N/A     | Provides runtime analytics and logging.                   | Using built-in monitoring tools and self-hosted solutions is the most cost-effective approach while staying under budget. |
