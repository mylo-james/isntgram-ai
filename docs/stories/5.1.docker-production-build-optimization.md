...
1. Per-app production Dockerfiles are optimized for production (`apps/web/Dockerfile.prod`, `apps/api/Dockerfile.prod`)
   with:
...
- [ ] **Task 1: Create Optimized production Dockerfiles** (AC: 1)
  - [ ] Create multi-stage Dockerfiles for web/api (`apps/web/Dockerfile.prod`, `apps/api/Dockerfile.prod`)
