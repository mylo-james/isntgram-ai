...
- [x] **Task 5: Implement Demo Session Management** (AC: 3, 5)
  - [x] Add demo session handling in Auth.js callbacks (mark user as demo via token/session claim)
  - [x] Implement demo user state management in Redux (e.g., `isDemoUser` flag)
  - [x] Add demo user identification in UI components (badge/tooltip) — indicators present in `EditProfileModal` and
        disabled states/tooltips in `ProfileActions`; global banner added in layout
  - [x] Add demo session expiration and cleanup — short-lived session configured; relies on default cleanup mechanisms
  - [x] Add demo mode indicator in navigation or profile header — banner at top of layout
...
- [ ] **Task 7: Create Comprehensive Tests** (Testing Requirements)
  ...
  - [x] Test demo authentication flow and session creation (unit/integration) — flow covered in web unit test and API
        integration test
  - [x] Test demo user identification and restrictions in UI components (e.g., `EditProfileModal` and `ProfileActions`)
  - [x] Create backend API tests for demo authentication (if Option A)
  - [x] Add E2E test for demo sign-in flow (Playwright)
