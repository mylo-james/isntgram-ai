...
4. A basic CI pipeline (e.g., using GitHub Actions) is created that installs dependencies and runs all tests on every push to the main branch.
...
- `apps/web/jest.config.cjs` - Enhanced with coverage configuration
- `apps/web/package.json` - Verified test scripts
- `apps/api/jest.config.cjs` - Enhanced with coverage and setup configuration
- `apps/api/package.json` - Verified test dependencies
- `package.json` - Added E2E and coverage scripts
